...
$(document).ready(() => {
    ...
    let dropzone = new Dropzone("#newFiles", {
    ...
    function updateSelectRecipients() {
        $.get(Urls["review:recipients"](analysis), (response) => {
            const selectedRecipients = JSON.parse(
                response["current_recipients"]
            );
            ...
            const selectedCCRecipients = selectedRecipients
                .filter(
                    (selected) =>
                        ...
                        !selectedMainRecipients.includes(
                            selected.fields.recipient
                        )
                )
                .flatMap((selected) => [selected.fields.recipient]);
    ...
    $("#finished").on("sortreceive", (e, ui) => {
        ...
        $.get(Urls["review:analysis_emails"](analysisId), (response) => {
            ...
            Swal.fire({
                title:
                    "Se enviara un correo a los siguientes contactos, ¿esta seguro?",
                html: text,
                input: "select",
                inputOptions: {
                    1: "ESP",
                        ...
                        success: (response) => {
                            if (response.status === "OK") {
                                ...
                                switch (response.code) {
                                    ...
                                    case 2:
                                        toastr.warning(
                                            "Ocurrio un error enviando el correo."
                                        );
                                        break;
                                }
                            }
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax(Urls["review:mail_list"](mailListPk), {
                    ...
                    success: (response) => {
                        toastr.success(
                            `Lista de correos ${mailListName} actualizada.`
                        );
                    },
                    error: (err) => {
                        toastr.error("Ha ocurrido un error.");
