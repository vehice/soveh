...
function loadSamples(samples){
  ...
  //   $('.stain-select-'+ item).select2();
    ...
    //     });
    //   } else {
    //     $.each(data_step_3.samples, function(i, item2){
...
function getSampleAvailableOrgans(sid){
  var organs;
  $.each(data_step_3.samples, function(index, value){
    if (sid == value.id){
...
function addExamToSamples(exam){
    var exam_object = data_step_3.exams.find(x => x.id == exam.id);
    $('#samples_table .samples_exams').each( function(i){
        ...
        if ( $('#switchery')[0].checked ){
            ...
            $.each(data_step_3.samples, function(i, item){
